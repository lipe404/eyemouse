...
class CalibrationUI:
    def __init__(self, root, calibration_manager, get_latest_gaze_fn, on_complete):
        ...
        self.points = [
            (margin, margin), (cx, margin), (w - margin, margin),
            (margin, cy),     (cx, cy),     (w - margin, cy),
            (margin, h - margin), (cx, h - margin), (w - margin, h - margin)
        ]
        ...
        # Elementos da UI
        self.instruction = self.canvas.create_text(cx, cy - 100, text="Olhe para o ponto vermelho", fill="white", font=("Arial", 24))
        self.progress_bar_bg = self.canvas.create_rectangle(cx - 100, cy + 100, cx + 100, cy + 120, outline="white")
        self.progress_bar_fill = self.canvas.create_rectangle(cx - 100, cy + 100, cx - 100, cy + 120, fill="green", outline="")
    ...
    def show_point(self):
        ...
        # Desenhar alvo
        r = 20
        self.canvas.create_oval(x-r, y-r, x+r, y+r, fill="red", outline="white", tags="target")
    ...
    def update_progress(self, percent):
        ...
        self.canvas.coords(self.progress_bar_fill, cx - 100, cy + 100, cx - 100 + filled_width, cy + 120)

    def finish_calibration(self):
        self.canvas.delete("all")
        self.canvas.create_text(self.width//2, self.height//2, text="Calibrando...", fill="white", font=("Arial", 24))
        self.window.update()
        
        success = self.calib_manager.compute_calibration()
        
        if success:
            self.canvas.create_text(self.width//2, self.height//2 + 50, text="Sucesso!", fill="green", font=("Arial", 20))
        else:
            self.canvas.create_text(self.width//2, self.height//2 + 50, text="Falha na calibração", fill="red", font=("Arial", 20))
